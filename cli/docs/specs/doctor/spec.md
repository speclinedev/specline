...
5. **Structural checks (canon 2.3 structural subset; per-rule `scope` per the
   ...
   is `scope: repo`).**
   - Every `docs/specs/<slug>/` contains `spec.md` and `relations.md`.
   - Frontmatter parses, and its `slug` matches the directory name.
   - `ratified` and `building` specs carry `ratified_by` and `ratified_at`
     (B3).
   - `relations.md` forward edges to repo-local slugs resolve at some lifecycle
     stage; cross-repo (`repo:slug`) edges are warn-only; edges to
     `killed` slugs warn.
...
6. **Slug integrity (`scope: repo`).** A spec's identity is its slug — the folder
   name — unique across `specs/`, `knowledge/`, and `archive/` (`SLUG-DUPLICATE`);
   no reused slugs. There is no id counter. A ratified slug is frozen by
   *consequence*, not a dedicated rule: renaming a spec anything depends on dangles
   its inbound edges (`RELATION-DANGLING`), and renaming an archived one trips
   `ARCHIVE-EDITED` — so a rename that would break a reference cannot pass the gate.
   An abandoned spec is **archived** (`status: killed`, reason "abandoned"), never
   bare-deleted (canon discipline), so every ratified slug resolves in
   `specs/ ∪ knowledge/ ∪ archive/` forever.
...
7. **Stable rule identifiers.** Every check emits a documented, stable
   ...
   `SLUG-DUPLICATE`, `LINK-DANGLING`, `ARCHIVE-EDITED`,
...
`scope` and `downgradable` are orthogonal: `FRONTMATTER-SLUG-MISMATCH` is
`scope: spec, downgradable: false`; a missing `relations.md` is
`scope: spec, downgradable: true`; a duplicate slug is

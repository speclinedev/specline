...
a thin wrapper over the same engine and output contract as the CLI (`doctor`)
...
- `doctor` — the engine, output contract, and gate-mode/exit-code semantics
  this wraps. Any divergence in pass/fail is a bug here, not in the engine.
- `specline-init` — generates the `.github/workflows/specline.yml` that
  invokes this action.

...
// The two authored files that carry the canon pin. They are scaffolded once and
// never regenerated by sync, so a canon bump leaves them stale — `upgrade` is the
// sanctioned way to rewrite just the version token in each, in place.
const PIN_REWRITES: { rel: string; re: RegExp; replace: string }[] = [
  { rel: "specline.yml", re: /^(canon:[ \t]*)\S+/m, replace: `$1${CANON}` },
  {
